...
gen_vless_xhttp() {
    ...
    echo -e "${Font_Cyan}正在配置 VLESS-XHTTP-TLS (增强兼容模式)...${Font_Suffix}"

    # 1. 核心配置：使用 xhttp 并保持默认 mode (auto) 以适配 26.x 核心
    cat <<EOF > $config_path
{
    "log": { "loglevel": "warning" },
    "inbounds": [{
...
EOF
    ...
    # 2. Caddy 配置：必须启用 h2c 传输以支持 XHTTP 的 HTTP/2 流量
    echo "$domain {
    tls {
        protocols tls1.2 tls1.3
    }
