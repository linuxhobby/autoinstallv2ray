...
# ====================================================
# 将军自持版 V2ray_install.sh (由 V6.3 升级)
# 1. 17 协议全阵列，VLESS-WS-TLS 居首
# 2. 强制同步上海时区
# 3. 集成 查看/新增/删除 管理逻辑 (修复删除逻辑中的 local 语法错误)
# 4. 新增 BBR 战略加速模块 (彻底修复 printf 选项解析错误)
# 5. 链接：https://raw.githubusercontent.com/linuxhobby/auto-install-v2ray/refs/heads/master/install_v2.sh
# ====================================================
...
init_system() {
    ...
    if [ ! -f "$V2_BIN" ]; then
        ...
        curl -L -o /tmp/v2.zip "https://github.com/v2fly/v2ray-core/releases/download/${latest_ver}/v2ray-linux-${arch}.zip"
...
deploy_services() {
    ...
    if [[ "$is_tls" == "true" ]]; then
        if ! command -v caddy &> /dev/null; then
            _yellow ">>> 正在构筑 Caddy 证书防线 (GPG 修复版)..."
            # 彻底清理
...
EOF
        systemctl daemon-reload # 确保 systemd 识别最新的配置
...
while true; do
    ...
    printf -- "\033[31m       特征码：666666661       \033[0m\n"
    printf -- "\033[31m===============================================\033[0m\n"
    ...
    case $main_opt in
        ...
        2)
            while true; do
                clear
                printf -- "========= 17 协议战术矩阵 (v2ray_install) =========\n"
                printf -- "\033[1;31m  1) VLESS-WS-TLS       [王牌：最稳且支持CDN]\033[0m\n"
                printf -- "\033[1;31m  2) VLESS-gRPC-TLS     [极速：抗封锁性能优异]\033[0m\n"
